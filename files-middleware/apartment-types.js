...
const bucket = gc.bucket('binadari_apartment_types');

exports.createNewDir = async () => {
  let files;
  try {
  ...
  let lastIndexName = filenames[filenames.length - 1];
  let lastIndex = lastIndexName.split('type_')[1].split('/')[0];
  let newIndex = parseInt(lastIndex, 10);
  newIndex++;
  return `type_${newIndex.toString().padStart(3, '0')}`;
};
